...
void CompMesh2D::initialize( std::map<int, CompNode>& nodes_in,
                              int firstNode_in,
                              int lastNode_in,
                              ...
                              , double* color )
{
  ...
  for(it_cells=cells_in.begin();
    ...
    triangles.back()->GetPointIds()->SetId ( 0, (*it_cells)[0]-firstNode_in );
    triangles.back()->GetPointIds()->SetId ( 1, (*it_cells)[1]-firstNode_in );
    triangles.back()->GetPointIds()->SetId ( 2, (*it_cells)[2]-firstNode_in );
  
    domain->InsertNextCell ( triangles.back() );
  }
...
//   anActor->GetProperty()->SetDiffuseColor(0.0, 0.3, 0.3);
  
}

void CompMesh2D::readBoundary(ifstream& input, int segmentsNumber )
{
  int n1, n2, j;  
...
void CompMesh2D::addToRender(vtkRenderer * renderer_in)
{
  if(anActor) renderer_in->AddActor( anActor );
}

void CompMesh2D::removeFromRender(vtkRenderer * renderer_in)
{
  if(anActor) renderer_in->RemoveActor( anActor );
}

void CompMesh2D::updatePoints()
{
  for(std::map<int,CompNode*>::iterator it = nodes.begin();
      it != nodes.end();
      ++it)
...
void CompMesh2D::readEnergy( std::ifstream & input, int timeSize )
{
...
void CompMesh2D::readResults( std::ifstream & input, int timeSize )
{
...
void CompMesh2D::readTemps( std::ifstream & input, int timeSize )
{
  double res1, res2(0.);
...
void CompMesh2D::drawScalarField(int index, int step)
{
//  double min_max[2];
  currentIndex = index;
...
void CompMesh2D::updateScalarField(int step)
{
//  double min_max[2];
  if( scalarFields.size() > 0){
    profile->GetPointData()->SetScalars( scalarFields[step][currentIndex] );
...
void CompMesh2D::setLookUpTable( vtkLookupTable* table_in )
{ 
//   aDataSetMapper->SetScalarRange( table_in->GetRange() );
  aDataSetMapper->SetLookupTable( table_in); 
}


void CompMesh2D::findMinMaxScalars()
{
  double min_max[2];
